...
THRESHOLD_DIST = 20  # 到达航点阈值（米）
TARGET_WP_INDEX = 5   # 第5号航点

# 备用坐标
FALLBACK_LAT = 30.111111
FALLBACK_LON = 120.222222
...
def try_process_pending():
    ...
    for item in pending_pixels:
        ...
        if gps:
            ...
            try:
                num_int = int(item["digits"])
                number_memory.number_counts[num_int] = number_memory.number_counts.get(num_int, 0) + 1
                number_memory.gps_data.setdefault(num_int, []).append((lat, lon))
            except Exception:
                pass
        else:
